...
## 接收消息
客户端请求的回包，通过异步等待的方式接收。`ProcessInnerSender.Update`方法中，会检测是否有待发送的协议，如果有，那么就遍历这些协议，调用`HandleMessage`方法
```CSharp
...
{  
    ...
    {        
        ...
        {            
            IResponse resp = MessageHelper.CreateResponse(request.GetType(), 
                                                          request.RpcId, 
                                                          ErrorCore.ERR_NotFoundActor);  
...
```
`HandleMessage`方法中，如果一个带发送的协议是具有回包的，那么会使用`HandleIActorResponse`另外处理。直接发送的协议，会获取到`Root Scene `