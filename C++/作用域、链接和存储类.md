...
```cardlink
url: https://www.cnblogs.com/codefuturedalao/p/14400825.html
title: "作用域、链接属性和存储类型 - 扶磐 - 博客园"
description: "最近在读《程序员的自我修养——链接、装载与库》，感觉自己当初学习C的时候，对extern、static等关键字了解不是特别清晰，因此重温了一遍《C和指针》中关于作用域、链接属性和存储类型的相关部分，加上了自己的理解，用博客记录一下。 作用域 当变量在程序的某个部分被声明时，它只有在程序的一定区域才能"
...
```cardlink
url: https://zhuanlan.zhihu.com/p/476697014
title: "程序详细编译过程（预处理、编译、汇编、链接）"
description: "程序编译的整体流程以下面这个简单的函数为例 #include&lt;stdio&gt; int main() { printf(&#34;hello, world &#34;); return 0; }hello程序的生命周期是从一个 源程序（或者说源文件）开始的，即程序员通过编辑器…"
