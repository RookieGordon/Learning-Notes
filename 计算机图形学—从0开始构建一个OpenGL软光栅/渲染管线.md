...
对于Shader而言，每个Mesh的一次绘制，都会调用很多次。每次VertexShaderi调用都会传入不同顶点的属性；每次FragmentShaderi都会传入不同片元数据；但是每个顶点都会使用相同的model/view/projection矩阵，每个Fragment都会使用相同的光照数据。如果每次都创建新的数据，无疑会增大kai
