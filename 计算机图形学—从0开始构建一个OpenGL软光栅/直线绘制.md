...
\end{align}$$
通过对$d_{0}$和$d_{1}$的长度判断出直线距离上下两个像素谁更近：$d_{0}-d_{1} = 2k(x_{i} + 1) - 2y_{i} - 1 + 2b$，如果结果大于0则，下面的像素离直线更近，反之则上面的像素更近。
...
\end{align}$$
由于直线斜率$k = \frac{\Delta y}{\Delta x}$，所有$y_{i} = \frac{\Delta y}{\Delta x}x_{i}+b$，带入有：
$$\begin{align}
p_{i} & = 2\Delta yx_{i} - 2\Delta x(\frac{\Delta y}{\Delta x}x_{i} + b ) + (2\Delta y + 2b\Delta x - \Delta x) \\
p_{i} & = 2\Delta y - \Delta x  &(公式1)
\end{align}$$
...
& 有：p_{i + 1} - p_{i} = 2\Delta y - 2\Delta x(y_{i + 1} - y_{i}) &(公式2)
\end{align}$$
通过公式1和公式2，就能实现算法的迭代，转换成伪代码如下：
```Cpp
x = x1, y = y1;
// 这里的deltaX和deltaY是常量，只和像素点的大小有关
p = 2 * deltaY - deltaX;  // 公式1
While(x < x2){
	drawPoint(x, y);
	x++;
	if(p >= 0){
