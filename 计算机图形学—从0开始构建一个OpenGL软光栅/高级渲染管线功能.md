...
只需要修正视图空间下O点的重心坐标与NDC空间下O点坐标的关系即可！

## 透视修正的推导

推导从摄像机视图到NDC空间，重心坐标发生了什么变化

P矩阵是$4\times 4$投影矩阵，可以分解为：$3\times 3$矩阵M，3维列向量t，以及最后一行数字(abcd)，A点经过变换后有：
$$\begin{align}
A_{c} & = \begin{bmatrix}    
M &\vec{t}\\    
abc & d
...
摄像机空间中的三角形中一点$\vec{O} = \alpha \vec{A} + \beta \bar{B} + \gamma \vec{C}$，NDC中，O'为：$\vec{O'} = \alpha \vec{A'} + \beta \bar{B'} + \gamma \vec{C'}$，由此可得：O'也是O经过投影和透视除法所得：
$$\begin{align}
O' = 
\begin{bmatrix}    
...
\end{bmatrix} 
= \begin{bmatrix}    
\frac{M\times (\alpha \vec{A} + \beta  \vec{B} + \gamma  \vec{C}) + \vec{t}}{\omega _{A}} \\
1 
\end{bmatrix}  
\end{align}$$
联立等式有：
$$\begin{align}
O' & = \frac{M\times (\alpha \vec{A} + \beta  \vec{B} + \gamma  \vec{C}) + \vec{t}}{\omega _{A}} \\ 
