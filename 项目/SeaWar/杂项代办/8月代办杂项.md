---
tags:
  - SeaWar/杂项代办/8月代办杂项
  - mytodo
type: Project
project: SeaWar
projectType: Task
fileDirPath: 项目/SeaWar/杂项代办
dateStart: 2025-08-01
dateFinish: 2025-08-30
finished: false
displayIcon: pixel-banner-images/项目任务.png
---
- [ ] 服务器推送协议到指定svn仓库，客户端分支绑定svn仓库，一键拉取协议
- [ ] 本地打包功能，环境配置都放到git仓库中去；
- [ ] 本地做一个引用缓存，原本是A是通过guid引用B，再维护一个A通过路径引用B的缓存。如果guid丢失，并且路径也找不到就将引用设置为None。
- [ ] 进一步完善AssetBundleCollectorSetting检查工具，增加资源删除的检查。并且考虑是否将其放到工程外？或者打包前自动创建？
- [ ] 批量处理掉，角色动画中的scale数据。已有工具
- [ ] 剔除fbx中mesh的顶点色，切线和多余的UV
- [ ] 自动降低画质
- [x] 下载app部分，支持后台下载
- [x] 游戏初始化各步骤的耗时，敏感信息输出成debug
- [ ] 通知信息整合到一条
- [ ] 通知@大头
- [ ] 删除空目录和其meta（只有meta，有目录也有meta）
- [x] 服务器维护公告界面修改
- [x] 强更弹窗去掉退出按钮
- [ ] 动画资源提取的bug修复
- [ ] 在热更通知中，新增一个热更资源对比的报告地址
- [x] 外网后台请求压测
- [ ] fix_progress2修复了进度条显示的bug
Jenkins项目：[SeaWar/android_develop] 运行成功！  
构建类型：热更  
渠道：101-内网develop  
版本号：App v101.0.72 Res v25.08.19.2  

<color=#ff0000>[热更大小： 20708K](http://192.168.4.14:8080/job/SeaWar/job/android_develop/433/artifact/artifacts/SeaWar-android_develop-develop_bundle_diff.txt)

  
耗时： 00:04:24



