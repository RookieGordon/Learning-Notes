...
### 关键点：Normalized 必须在签名之前

```bash
#!/bin/bash
# === Unity 项目 CI 构建脚本示例 ===

# Step 1: Unity 构建（输出已签名 APK，但我们需要重新处理）
unity -batchmode -executeMethod BuildScript.BuildAndroid
UNITY_APK="build/app.apk"

# Step 2: Normalized（会删除 V2 签名，规范化压缩数据）
./ApkNormalized "$UNITY_APK" "build/app-normalized.apk"

# Step 3: 重新签名（使用与 Unity 相同的 keystore）
apksigner sign \
    ...
    --in "build/app-normalized.apk" \
    --out "build/app-release.apk"

# Step 4: 验证签名
apksigner verify -v "build/app-release.apk"
...
OLD_APK="releases/v1.0.0/app-release.apk"  # 线上旧版本
NEW_APK="releases/v1.1.0/app-release.apk"  # 新版本
OUTPUT="patches/v1.0.0_to_v1.1.0"

mkdir -p "$OUTPUT"

# Step 1: 直接 Diff（两个 APK 都已经是 Normalized + 签名状态）
./ZipDiff "$OLD_APK" "$NEW_APK" "$OUTPUT/update.patch"

# Step 2: 验证 Patch（用旧 APK 合成，对比新 APK）
./ZipPatch "$OLD_APK" "$OUTPUT/update.patch" "$OUTPUT/test.apk"

# Step 3: Hash 校验
NEW_HASH=$(sha256sum "$NEW_APK" | cut -d' ' -f1)
TEST_HASH=$(sha256sum "$OUTPUT/test.apk" | cut -d' ' -f1)

if [ "$NEW_HASH" == "$TEST_HASH" ]; then
    echo "✅ Patch 验证通过"
    # 生成 manifest.json 供客户端使用
    echo "{
      \"from_version\": \"1.0.0\",
      \"to_version\": \"1.1.0\",
      \"patch_hash\": \"$(sha256sum $OUTPUT/update.patch | cut -d' ' -f1)\",
      \"old_apk_hash\": \"$(sha256sum $OLD_APK | cut -d' ' -f1)\",
      \"new_apk_hash\": \"$NEW_HASH\"
...
public void applyPatch() {
    // 1. 获取已安装 APK 路径
    String oldApkPath = getPackageManager()
        .getApplicationInfo(getPackageName(), 0).sourceDir;
    
    // 2. 下载 patch 文件并校验 hash
    String patchPath = downloadAndVerifyPatch();
    
    // 3. 合成新 APK
    String newApkPath = getCacheDir() + "/new.apk";
    int result = ApkPatch(oldApkPath, patchPath, newApkPath, 
                          0, null, 1);
    
    if (result == 0) {  // PATCH_SUCCESS
        // 4. 校验新 APK 的 hash
        if (verifyHash(newApkPath, expectedHash)) {
            // 5. 安装新 APK
