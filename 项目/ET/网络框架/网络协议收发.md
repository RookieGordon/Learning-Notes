...
```
在客户端中，`ActorId`用于记录消息的目的地。`Address`用于标记`Fiber`的地址。在客户端收发的消息，会被包装成`MessageInfo`结构体，消息内容放在`MessageObject`当中
```CSharp
...
```
`RpcId`用于标记发送的`Actor`的`RpcId`，该值是自增的，用于标记协议。
...
```
`Call`用于有回包的`Actor`消息的发送，`SendInner`方法会对`ActorId`和当前`Fiber`做检查，规定`Actor`消息能发往同一个进程
```CSharp
// 如果发向同一个进程，则扔到消息队列中
...
```
通过检查的消息，会添加到`MessageQueue.Instance.messages`字典中去
