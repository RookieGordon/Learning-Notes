...
>3、客户端发送Actor消息，标记的发送来源地是`NetClient Fiber`，即`ClientSenderComponent.netClientActorId`
## Call和Send发送Actor消息
```CSharp
...
```
`Call`用于有回包的`Actor`消息的发送，`SendInner`方法会对`ActorId`并且只能发往同一个进程。
，会使用`MessageQueue`单例对象，将需要发送的消息，添加到队列中去。
