...
1. [[协议对象|定义一个不需要返回的消息，没有其他需要就不用额外标记]]
...
服务器的消息收发流程就会比较复杂，其包含内部消息的收发和对客户端消息的收发。如下的时序图是服务器对客户端的消息收发流程：

![[（图解3）服务器消息流转.drawio.png]]

服务端`NetComponet`收到消息后，会派发`NetComponentOnRead`事件出去，有两个处理的地方——`NetComponentOnReadInvoker_Relam`和`NetComponentOnReadInvoker_Gate`。

## Realm和Gate

这两者的区别和各自职责如下：

### 1. Gate Fiber 的职责与 NetComponentOnReadInvoker_Gate

- **Gate Fiber** 主要负责**客户端的连接、认证、会话管理**，是客户端进入服务器的第一站。
- 典型职责：
  - 处理客户端的登录、注册、心跳、断线重连等
  - 维护 Session，分配 Player
  - 作为消息转发枢纽，将需要进一步处理的消息（如游戏逻辑、房间逻辑）转发到其他 Fiber（如 Map、Room）
- **NetComponentOnReadInvoker_Gate** 负责分发 Gate 收到的所有消息，按消息类型（ISessionMessage、IRoomMessage、ILocationRequest等）决定是本地处理还是转发到其他 Fiber 或 Actor。

### 2. Realm Fiber 的职责与 NetComponentOnReadInvoker_Realm

- **Realm Fiber** 主要负责**账号体系、网关分配、全局认证**，是客户端登录流程的前置服务。
- 典型职责：
  - 处理账号注册、登录验证
  - 分配 Gate 服务器地址（让客户端知道连哪个 Gate）
  - 可能还会做全局的账号状态管理、踢号等
- **NetComponentOnReadInvoker_Realm** 负责分发 Realm 收到的消息，通常只处理与账号、登录、分配 Gate 相关的请求，逻辑比 Gate 更简单、更专一。

### 3. 主要区别

- **Realm** 是“账号/登录中心”，只处理登录、注册、分配 Gate 的请求，**不维护玩家 Session**，不处理游戏逻辑。
- **Gate** 是“网关/会话中心”，负责维护与客户端的长连接、Session、Player，处理登录后的所有客户端请求，并根据类型决定本地处理还是转发到后端 Fiber（如 Map、Room）。
- **NetComponentOnReadInvoker_Realm** 只分发 Realm 相关的消息（如 C2R_Login），**NetComponentOnReadInvoker_Gate** 分发所有登录后客户端消息（如 C2G_EnterMap、C2M_TestRequest 等）。

---

**总结**：  
- 客户端登录时，先连 Realm，完成账号验证和 Gate 分配（由 NetComponentOnReadInvoker_Realm 处理）  
- 然后连 Gate，后续所有消息都走 Gate，由 NetComponentOnReadInvoker_Gate 按类型分发和转发  
- Realm 只做登录分发，Gate 负责所有后续会话和消息流转

## NetComponentOnReadInvoker_Gate

从了解消息收发的流程入手，我们先跳过`NetComponentOnReadInvoker_Realm`，重点关注`NetComponentOnReadInvoker_Gate`。

`NetComponentOnReadInvoker_Gate`是后端的收到客户端消息的第一个业务分发点。在`HandleAsync`函数中，根据[[协议对象#几种消息对象的扩展|消息的类型]]，进行分发处理。

先看`ISessionMessage`类型，会根据`sceneType`的类型，将协议数据转发到指定的Fiber上对应的事件处理器上进行处理，比如`C2G_EnterMapHandler`、`C2G_LoginGateHandler`等。

`IRoomMessage`、`ILocationMessage`和`ILocationRequest`都会调用各自的协议派发器做对应的转发操作。`MessageSender`和`MessageLocationSenderComponent`的作用，类似于`ProcessInnerSender`，从这里的得出，`ISessionMessage`类型的消息，应该只会在Gate Fiber上处理，不会转发到其他Fiber上去。
