...
服务端`NetComponet`收到消息后，直接派发到了`NetComponentOnReadInvoker_Gate`是第一个业务分发的入口，可以从此入手。

## NetComponentOnReadInvoker_Gate

`NetComponentOnReadInvoker_Gate`是后端的收到客户端消息的第一个业务分发点。在`HandleAsync`函数中，根据消息的类型，进行分发处理。